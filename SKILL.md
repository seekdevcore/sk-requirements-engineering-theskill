...
- 🎬 [`examples/template-user-story.feature`](examples/template-user-story.feature) — ready Gherkin file with 4 scenarios + Scenario Outline + sample step definitions (Python + TypeScript). Full **step-def bindings for 10 stacks** (pytest-bdd · behave · cucumber-js · cucumber-playwright · Reqnroll/SpecFlow · Behat · Cucumber-JVM/Java · godog/Go · cucumber-cpp/C++ · Cucumber wire protocol/pure C) in [`examples/feature-step-defs/`](examples/feature-step-defs/) — write the Gherkin once, bind it in any test stack
