...
| **pt-BR** | 🟢 Complete | **Full pt-BR translation at [`translations/pt-BR/`](./translations/pt-BR/)** — a faithful mirror of the en-CA source kept current through v1.22.0 (`SKILL.md` incl. §0 first-run / §3.1 SDD / EARS, `references/` 01–13, `examples/` (8, incl. SaaS/fintech/government case studies) + `feature-step-defs/`). The original *"IFPB"* course language; authoritative for compliance/audit reference. |
...
>
> **Se você lê português**: o conteúdo completo em pt-BR vive em [`translations/pt-BR/`](./translations/pt-BR/) — espelho fiel da fonte en-CA, mantido até a v1.22.0 (SKILL.md com §0/§3.1/EARS, referências 01–13, exemplos (8, incl. casos SaaS/fintech/governo) + feature-step-defs/). Continua íntegro e atualizado.
...
│       └── openproject.md         (optional OpenProject bridge — backlog ↔ work packages via REST API round-trip; Excel .xlsm = Windows-only fallback)
├── assets/
...
│   │   ├── openproject-api.py          (PRIMARY — pull/push docs/backlog/ ↔ OpenProject REST API v3; lockVersion, retry, idempotent)
│   │   └── project-to-openproject.py   (Windows-only Excel-sync fallback — docs/backlog/ → work-package CSV + XLSX)
